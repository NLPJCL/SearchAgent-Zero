...
def process_single_row(row, current_split_name, row_index):
    ...
    prompt = [{"role": "system", "content": DEFAULT_SYSTEM_CONTENT}, {"role": "user", "content": question}]
            ...
            try:
                ...
                if split=="test":
                    if "data_source" in df_raw.columns:
                        print("Filtering data: keeping max 50 rows per data_source...")
                    # Group by data_source and take the top 50
                        df_raw = df_raw.groupby("data_source").head(500)
                    # Reset index to ensure clean row numbering (optional but recommended)
                        df_raw = df_raw.reset_index(drop=True)
                        print(f"Filtered down to {len(df_raw)} rows total.")
                ...
                output_file_path = os.path.join(local_save_dir, f"{split}_search_r1.parquet")
