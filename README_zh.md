...
- **原生 verl AgentLoop 多轮搜索 rollout**：基于 verl 的 AgentLoop 抽象接入搜索工具，在 actor rollout 阶段完成多轮搜索、阅读、推理与答案生成。
- **可复现 Search-R1 与 ASearch recipe**：提供短程 Search-R1 复现脚本，以及面向长程搜索的 Qwen3-8B ASearch 训练脚本。
- **本地 dense retriever 与自定义搜索服务**：默认支持 Search-R1 风格的本地 E5 + Wikipedia 检索服务，也可以替换为自有检索或搜索 API。
